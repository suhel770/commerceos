...
export default function MediaSection({
  ...
  const editGallery = () => {
    openFieldEditor({
      ...
      label: "Image URLs",
      value: (product.gallery ?? [product.image])
        .filter(Boolean)
        .join(", "),
      inputType: "textarea",
      marketplace: "Marketplace media",
      description:
        "Add comma-separated image URLs. CommerceOS uses these to prepare marketplace-specific image sets.",
      onSave: (value) =>
        updateDraft({
          gallery: value
            .split(",")
  ...
  const editVideo = () => {
    openFieldEditor({
      ...
      marketplace: "Marketplace media",
      description:
        "Used where video is supported, such as Shopify and selected marketplace categories.",
      onSave: (value) =>
        updateDraft({ video: value }),
    });
  };
...
function StatCard({
  ...
  return (
    <button
      type="button"
      onClick={onEdit}
      className={`rounded-xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-sm ${styles[color]}`}
    >
